...
mod tests {
    ...
    fn test_grid(dim: (usize, usize), pop: Vec<usize>, num_districts: usize) {
        ...
        let objective = |assignment: &[usize]| {
            ...
            for (cell, &district) in zip(cells.iter(), assignment.iter()) {
                districts[district] =
        ...
        let mut annealer = Annealer::from_starting_state(
            ...
            ANNEAL_POP_THRESH,
            objective,
            |x| (x * (-T0) as f64).max(0.5),
        );

        let (final_score, assignment, hist) = annealer.anneal(1000, NUM_THREADS);
        dbg!(hist);
